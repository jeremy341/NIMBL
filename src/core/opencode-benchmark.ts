  ...
  const lane = async (items: typeof work) => {
    for (const item of items) {
      ...
      runs.push({
        ...
        // OpenCode reports step input excluding cache-read tokens. Normalize the
        // record to NIMBL's full-input convention so totalTokens is comparable.
        inputTokens: tokens.input + tokens.cacheRead,
        outputTokens: tokens.output,
        totalTokens: tokens.input + tokens.cacheRead + tokens.output + tokens.reasoning,
        noCacheTokens: tokens.input,
        cacheReadTokens: tokens.cacheRead,
        cacheWriteTokens: tokens.cacheWrite,
        billedTokens: tokens.input + tokens.output + tokens.reasoning,
        referenceCostUsd: estimateReferenceCost(tokens.input + tokens.cacheRead, tokens.output),
